...
//***首页  大乐透下单详情
    .controller ('BigLottoCtrl', function ($scope, $rootScope, $state, $ionicPopover, $interval, $ionicPopup, $cordovaToast, $getInfoService, $util, $timeout) {
        //设置红球和篮球号码
        $scope.numDataRed = [];
        $scope.numDataBlue = [];
        ...
        $scope.saveBallSelect = function () {
            ...
            if (filterDataRed.length == 5 && filterDataBlue.length == 2) {//判断用户未选择号码时点击确定无效
                ...
                //$cordovaToast.showShortCenter ("订单已提交到我的订单")
                    .then (function (success) {
                        // success
